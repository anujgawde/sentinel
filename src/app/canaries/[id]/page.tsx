        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              {plan.mode} workflow &middot;{" "}
              {plan.schedule === "manual" ? "Manual trigger" : plan.schedule}
            </p>
          </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-emerald-700/70 mt-0.5">
                    The latest run found no drift. You can now resolve this
                    alert.
                  </p>
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            {latestRun ? (
              <div className="space-y-1.5">
                <p className="text-2xl font-medium">
                  {latestRun.artifacts.timing.totalMs}
                  <span className="text-sm text-muted-foreground font-normal">
                    ms
                  </span>
                </p>
                <p className="text-xs text-muted-foreground">
                  {timeAgo(latestRun.endedAt)}
                </p>
              </div>
            ) : (
        ...
        <Card>
          ...
          <CardContent className="px-5 pb-4">
            <div className="space-y-1.5">
              <p
                className={`text-2xl font-medium ${latestRunFindings.length > 0 ? "text-destructive" : ""}`}
              >
          ...
          <CardContent className="px-5 pb-5">
            <div className="space-y-3">
              {latestRunFindings.map((finding) => {
                const c = driftConfig[finding.type] || {
                  label: finding.type,
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        Base severity {finding.severity} &middot; Overall{" "}
                        {Math.min(
                          100,
                          Math.round(
                            finding.severity *
                              (plan.mode === "login" ? 1.2 : 1.0),
                          ),
                        )}
                        /100 ({plan.mode} workflow 1.2x)
                      </p>
                    </div>
                  </div>
                );
        ...
        <Card>
          ...
          <CardContent className="px-5 pb-5">
            <div className="grid grid-cols-1 gap-3 text-sm">
              {[
                {
                  label: "DOM Fingerprint",
                  value: baseline.artifacts.domFingerprintHash,
                },
                {
                  label: "Step Shape",
                  value: baseline.artifacts.stepShapeHash,
                },
                {
                  label: "Redirect Hosts",
                  value: baseline.artifacts.redirectHostsHash,
                },
              ].map((item) => (
                <div
                  key={item.label}
              ...
              <div className="flex items-center justify-between gap-4 py-1.5">
                ...
                <span className="font-mono text-xs">
                  {baseline.artifacts.redirectHosts.join(", ")}
                </span>
              </div>
              <div className="flex items-center justify-between gap-4 py-1.5">
                <span className="text-muted-foreground">Baseline Timing</span>
                <span className="font-mono text-xs">
                  {baseline.artifacts.timing.totalMs}ms
                </span>
              </div>
            </div>
          </CardContent>
            ...
            <div className="flex gap-1 items-end h-12 pt-10">
              {runs
                ...
                .map((run) => {
                  ...
                  const maxSev = hasDrift
                    ? Math.max(...run.findings.map((f) => f.severity))
                    : 0;
                  const bg =
                    maxSev >= 60
                      ? "bg-destructive"
                  ...
                  return (
                    <div key={run.id} className="group relative flex-1 max-w-4">
                      <div
                        className={`h-10 rounded-sm ${bg} transition-opacity hover:opacity-80`}
                      />
                      <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 hidden group-hover:block bg-popover border rounded-md shadow-md p-2.5 text-xs whitespace-nowrap z-50">
                        ...
                        <p className="text-muted-foreground">
                          {run.findings.length} findings &middot;{" "}
                          {run.artifacts.timing.totalMs}ms
                        </p>
                      </div>
                    </div>
                  );
